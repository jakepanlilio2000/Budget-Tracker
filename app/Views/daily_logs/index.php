    ...
    <?php foreach ($logs as $log): ?>
        <div class="card glass">
            <div class="flex-between">
                ...
                <span class="badge"
                    style="background: var(--border-color);"><?= e($log['mood_context'] ?: 'Neutral') ?></span>
            </div>
            <div class="sensitive-data"
                style="margin-top: 1rem; font-size: 1.2rem; font-weight: bold; color: var(--danger);">
                - <?= number_format($log['total_spent'], 2) ?>
            </div>
            <?php if ($log['notes']): ?>
                <p class="text-secondary" style="margin-top: 0.5rem; font-style: italic; font-size: 0.9rem;">
                    "<?= e($log['notes']) ?>"</p>
            <?php endif; ?>
        </div>
    <?php endforeach; ?>
...
<div id="logModal" class="modal-overlay" style="display: none;"
    ...
    <div class="modal-content glass" style="padding: 1.5rem;">
        ...
        <form method="POST" action="<?= url('/daily-logs/store') ?>" class="form-stack mt-3">
            <?= \App\Core\CSRF::field() ?>
            <div class="form-group"><label>Date</label><input type="date" name="log_date" value="<?= date('Y-m-d') ?>"
                    required></div>
            <div class="form-group"><label>Total Spent Today</label><input type="number" step="0.01" name="total_spent"
            ...
            <div class="form-group"><label>Notes / Reflections</label><textarea name="notes" rows="3"
                    placeholder="What drove today's spending?"></textarea></div>

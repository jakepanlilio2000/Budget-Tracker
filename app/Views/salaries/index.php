...
<div class="page-header flex-between">
    ...
    <div style="display:flex; gap:0.5rem;">
        <a href="<?= url('/salaries/export-csv') ?>" class="btn"
            style="background: var(--text-secondary); color:white;"><i class="fas fa-file-csv"></i> Export</a>
    ...
    <div class="card glass stat-card">
        ...
        <div class="stat-info">
            ...
            <h3 class="sensitive-data">
                <?= $baseSym ?>
                <?= number_format((float) ($analytics['total_earned'] ?? 0), 2) ?>
            </h3>
        </div>
    </div>
    <div class="card glass stat-card">
        ...
        <div class="stat-info">
            ...
            <h3 class="sensitive-data">
                <?= $baseSym ?>
                <?= number_format((float) ($analytics['total_extras'] ?? 0), 2) ?>
            </h3>
        </div>
    </div>
    <div class="card glass stat-card">
        ...
        <div class="stat-info">
            ...
            <h3 class="sensitive-data">
                <?= (int) ($analytics['total_payslips'] ?? 0) ?>
            </h3>
        </div>
    </div>
</div>
                ...
                <tbody>
                    <?php foreach ($salaries as $s): ?>
                        <tr>
                            ...
                            <td>
                                <?= $baseSym ?>         <?= number_format((float) $s['basic_salary'], 2) ?>
                            </td>
                            <td style="color: var(--success); font-weight: bold;">
                                <?= $baseSym ?>         <?= number_format((float) $s['net_pay'], 2) ?>
                            </td>
                            <td>
                                <?= e(date('M d, Y', strtotime($s['payment_date']))) ?>
                            </td>
                            <td>
                                <a href="<?= url('/salaries/show/' . $s['id']) ?>" class="link">View Details</a>
                            </td>
                            <td>
                                <div style="display: flex; gap: 0.5rem;">
                                    <a href="<?= url('/salaries/edit/' . $s['id']) ?>" class="btn btn-sm"

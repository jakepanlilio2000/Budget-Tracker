...
<div class="grid grid-3 mb-4">
    <div class="card glass stat-card">
        ...
        <div class="stat-info">
            ...
            <h3 class="sensitive-data"><?= $baseSym ?><?= number_format((float) ($analytics['total_earned'] ?? 0), 2) ?>
            </h3>
        </div>
    </div>
    <div class="card glass stat-card">
        ...
        <div class="stat-info">
            ...
            <h3 class="sensitive-data"><?= $baseSym ?><?= number_format((float) ($analytics['total_extras'] ?? 0), 2) ?>
            </h3>
        </div>
    </div>
    <div class="card glass stat-card">
                        ...
                        <tr>
                            ...
                            <td style="text-align: right;">
                                <div style="display: inline-flex; gap: 0.5rem;">
                                    <a href="<?= url('/salaries/show/' . $s['id']) ?>" class="btn btn-sm"
                                        style="background: var(--bg-glass-solid); border: 1px solid var(--border-color); color: var(--text-primary);"
                                        title="View Details">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <button class="btn btn-sm edit-salary-btn" style="background: var(--accent); color: white;"
                                        title="Edit" data-id="<?= $s['id'] ?>" data-employer="<?= $s['employer_id'] ?>"
                                        data-start="<?= e($s['pay_period_start']) ?>" data-end="<?= e($s['pay_period_end']) ?>"
                                        data-basic="<?= (float) $s['basic_salary'] ?>" data-bonus="<?= (float) $s['bonus'] ?>"
                                        data-overtime="<?= (float) $s['overtime_pay'] ?>"
                                        data-13th="<?= (float) $s['thirteenth_month'] ?>"
                                        data-date="<?= e($s['payment_date']) ?>" data-status="<?= e($s['status'] ?? 'paid') ?>"
                                        data-notes="<?= e($s['notes'] ?? '') ?>" data-allowances='<?= $allowancesJson ?>'
                                        data-deductions='<?= $deductionsJson ?>'>
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <form method="POST" action="<?= url('/salaries/delete/' . $s['id']) ?>"
                                        style="display:inline;"
                                        onsubmit="return confirm('Are you sure you want to delete this payslip?');">
                                        <?= \App\Core\CSRF::field() ?>
                                        <button type="submit" class="btn btn-sm"
                                            style="background: var(--danger); color: white;" title="Delete">
...
<div id="salaryModal" class="modal-overlay" style="display: none;" onclick="if(event.target===this) closeSalaryModal()">
    <div class="modal-content glass" style="padding: 1.5rem; max-width: 800px; max-height: 90vh; overflow-y: auto;">
        <div class="flex-between"
            ...
            <button class="btn-icon" onclick="closeSalaryModal()" style="font-size: 1.2rem;"><i
        ...
        <form method="POST" action="" id="salaryForm" class="form-stack">
            ...
            <h4
                style="margin-top: 0.5rem; color: var(--text-secondary); font-size: 0.9rem; text-transform: uppercase; letter-spacing: 0.05em;">
                Employer & Period</h4>
            ...
            <h4
                style="margin-top: 1rem; color: var(--text-secondary); font-size: 0.9rem; text-transform: uppercase; letter-spacing: 0.05em;">
                Earnings</h4>
            ...
            <div class="grid grid-2 mt-3">
                <div>
                    <div class="flex-between">
                        ...
                        <button type="button" class="btn btn-sm" style="background:var(--success);color:white;"
            ...
            <div class="flex-between mt-4" style="border-top: 1px solid var(--border-color); padding-top: 1rem;">
                <button type="button" class="btn" style="background: var(--text-secondary); color: white;"
                    onclick="closeSalaryModal()">Cancel</button>

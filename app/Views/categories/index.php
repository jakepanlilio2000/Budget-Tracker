    ...
    <div class="card glass" style="border-left: 4px solid <?= e($cat['color']) ?>;">
        <div class="flex-between">
            <div style="display: flex; align-items: center; gap: 0.75rem;">
                <div style="width: 40px; height: 40px; border-radius: 10px; background: <?= e($cat['color']) ?>20; color: <?= e($cat['color']) ?>; display: flex; align-items: center; justify-content: center;">
            ...
            <form method="POST" action="<?= url('/categories/archive/' . $cat['id']) ?>" onsubmit="return confirm('Archive this category?')">
                <?= \App\Core\CSRF::field() ?>
                <button type="submit" class="btn btn-sm" style="background:transparent; color:var(--text-secondary);"><i class="fas fa-archive"></i></button>
            </form>
        </div>
    </div>
    <?php endforeach; ?>
...
<h3 class="mt-4" style="color: var(--text-secondary);">Archived Categories</h3>
...
<!-- Add Category Modal -->
<div id="addCatModal" class="modal-overlay" style="display: none;" onclick="if(event.target===this)this.style.display='none'">
    <div class="modal-content glass" style="padding: 1.5rem; max-width: 500px;">
        ...
        <form method="POST" action="<?= url('/categories/store') ?>" class="form-stack mt-3">
            ...
            <div class="grid grid-2">
                ...
                <div class="form-group"><label>Color</label><input type="color" name="color" value="#3b82f6" style="height: 40px; width: 100%;"></div>
            </div>
            
            <div class="form-group">
                <label>Icon</label>
                <input type="hidden" name="icon" id="selectedIcon" value="fa-tag">
                <div id="iconGrid" style="display: grid; grid-template-columns: repeat(6, 1fr); gap: 0.5rem; max-height: 150px; overflow-y: auto; padding: 0.5rem; background: var(--bg-glass-solid); border-radius: 8px; border: 1px solid var(--border-color);">

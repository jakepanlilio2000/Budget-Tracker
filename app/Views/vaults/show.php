...
<div class="card glass mb-4">
    <div class="flex-between" style="margin-bottom: 1.5rem;">
        <div>
            ...
            <h1 class="sensitive-data" style="margin: 0; color: var(--accent);">
                <?= $sym ?><?= number_format((float) $vault['current_amount'], 2) ?></h1>
        </div>
        <div class="text-right">
            <span class="text-secondary">Target</span>
            <h2 class="sensitive-data" style="margin: 0;">
                <?= $sym ?><?= number_format((float) $vault['target_amount'], 2) ?></h2>
        </div>
    </div>

    <div
        style="background: var(--border-color); border-radius: 99px; height: 14px; overflow: hidden; margin-bottom: 1rem;">
        <div
            style="width: <?= $metrics['percentage'] ?>%; height: 100%; background: var(--accent); transition: width 0.5s;">
        </div>
    </div>
    ...
    <div class="grid grid-3" style="text-align: center; gap: 1rem;">
        ...
        <div>
            ...
            <h3 style="margin: 0;"><?= $metrics['estimated_months'] ? $metrics['estimated_months'] . ' mos' : 'N/A' ?>
            </h3>
        </div>
    </div>
    ...
    <?php if ($vault['status'] === 'active'): ?>
        <div class="flex-between mt-4" style="border-top: 1px solid var(--border-color); padding-top: 1rem;">
            ...
            <button class="btn" style="background: var(--danger); color: white;" onclick="openVaultModal('withdrawal')"><i
                    class="fas fa-arrow-up"></i> Withdraw</button>
            <form method="POST" action="<?= url('/vaults/status/' . $vault['id']) ?>"
                onsubmit="return confirm('Cancel this goal? Funds remain in the vault but it moves to history.')">
                <?= \App\Core\CSRF::field() ?>
                <input type="hidden" name="status" value="cancelled">
                <button type="submit" class="btn" style="background: var(--text-secondary); color: white;">Cancel
                    Goal</button>
            </form>
        </div>
    <?php endif; ?>
                ...
                <div class="timeline-item">
                    ...
                    <div class="glass" style="padding: 1rem; border-radius: 8px;">
                        ...
                        <?php if (!empty($t['notes'])): ?>
                            <p class="text-secondary" style="margin-top: 0.5rem; font-size: 0.9rem; font-style: italic;">
                                "<?= e($t['notes']) ?>"</p>
                        <?php endif; ?>
                    </div>
                </div>

...
<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 24px; align-items: start;">
    <div class="card">
        ...
        <div style="display: flex; flex-direction: column; gap: 12px;">
            <a href="<?= $basePath ?>/backups/<?= htmlspecialchars((string)$profile['id']) ?>/excel" class="btn primary" style="text-align: center; display: block; background: #217346; border-color: #217346; box-shadow: 0 0 10px rgba(33, 115, 70, 0.5);">📊 Export Excel</a>
            <a href="<?= $basePath ?>/backups/<?= htmlspecialchars((string)$profile['id']) ?>/json" class="btn ghost" style="text-align: center; display: block; border: 1px solid var(--border);">💻 Download as JSON (Dev Backup)</a>
        </div>
    </div>
    ...
    <div class="card" style="border-color: rgba(248, 81, 73, 0.3);">
        ...
        <form action="<?= $basePath ?>/backups/<?= htmlspecialchars((string)$profile['id']) ?>/wipe" method="POST" id="wipe-form" onsubmit="return confirm('⚠️ Are you absolutely sure? This will permanently wipe your ledger history and cannot be undone.');">
            ...
            <button type="submit" id="wipe-btn" class="btn danger" style="width: 100%;">⚠️ Wipe Ledger History</button>
        </form>
    </div>
</div>